...
namespace Jp.ParahumansOfTheWormverse.Lung
{
    public class SmashCardController : CardController
    {
        ...
        public override System.Collections.IEnumerator Play()
        {
            ...
            var e = DealDamageToLowestHP(TurnTaker.CharacterCard, 1, c => c.Alignment().Hero().Target() && c.IsInPlay, c => 2, DamageType.Melee);
            ...
            e = GameController.SelectAndDestroyCard(DecisionMaker, new LinqCardCriteria(c => c.Alignment().Hero() && (c.IsOngoing || c.DoKeywordsContain("equipment"))), optional: false, responsibleCard: Card, cardSource: GetCardSource());
            if (UseUnityCoroutines)
            {
                yield return GameController.StartCoroutine(e);
            }
