...
namespace Jp.ParahumansOfTheWormverse.Bitch
{
    public class FetchCardController : CardController
    {
        public FetchCardController(Card card, TurnTakerController controller) : base(card, controller)
        {
            ...
            SpecialStringMaker.ShowNumberOfCardsAtLocations(() => from ttc in GameController.FindTurnTakerControllersWhere((TurnTakerController ttc) => true, cardSource: GetCardSource()) select ttc.TurnTaker.Deck, new LinqCardCriteria((Card c) => (c.DoKeywordsContain("equipment") || c.DoKeywordsContain("device")), "equipment or device"));
        }
