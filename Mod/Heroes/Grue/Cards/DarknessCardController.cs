    ...
    public class DarknessCardController : CardController
    {
        ...
        public override void AddTriggers()
        {
            ...
            _reduceSourceTrigger = AddReduceDamageTrigger(
                dda => ! HasBeenSetToTrueThisTurn("FirstDamageDealtSource") && ! dda.DamageSource.IsGrueSource(this.FindGrueCharacterCard()) && Card.Location.IsNextToCard && dda.DamageSource.Card == GetCardThisCardIsNextTo(),
            ...
            _reduceTargetTrigger = AddReduceDamageTrigger(
                dda => ! HasBeenSetToTrueThisTurn("FirstDamageDealtTarget") && ! dda.DamageSource.IsGrueSource(this.FindGrueCharacterCard()) && Card.Location.IsNextToCard && dda.Target == GetCardThisCardIsNextTo(),
            ...
            // When Representative of Earth owns {GrueCharacter} he never takes a turn, so this
            // never comes due and the card stays in play - our turn taker is the environment,
            // whose turn end is not his.
            AddEndOfTurnTrigger(
                tt => tt == TurnTaker && TurnTaker.IsPlayer,
